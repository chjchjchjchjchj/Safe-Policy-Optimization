...
class CrossBarEnv(gym.Env):
    ...
    def __init__(self, env_id: str, **kwargs: dict[str, Any]) -> None:
        self.L = 5  # Number of layers
        self.num_agents = self.L
        self._count = 0
        self._num_envs = 1
        # self._observation_space = spaces.Box(low=0, high=1000, shape=(self.L,))
        # self.observation_space = gym.spaces.Tuple(tuple([spaces.Discrete(n=1000)] * self.L))
        # self.share_observation_space = self.observation_space
        # self._action_space = spaces.MultiDiscrete([3] * self.L)
        # self.action_space = gym.spaces.Tuple(tuple([spaces.Discrete(n=3)] * self.L))
        self.action_mapping = np.array([-1, 0, 1])

        self.observation_spaces = {}
        for i in range(self.num_agents):
            self.observation_spaces[f"agent_{i}"] = gym.spaces.Discrete(1000)
    ...
    def reset(
        ...
        obs = np.zeros(self.observation_space[0].shape, dtype=np.int32)
